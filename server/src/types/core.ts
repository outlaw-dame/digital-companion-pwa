...
// ─── Link Preview ─────────────────────────────────────────────────────────────
// Produced by the link processor for every URL found in user input.

export interface LinkPreview {
  url: string;            // Normalized URL (https only)
  domain: string;         // e.g. "github.com"
  title?: string;         // og:title or <title>
  description?: string;   // og:description, max 300 chars
  imageUrl?: string;      // og:image absolute URL
  favicon: string;        // Always present: /favicon.ico fallback
  isSafe: boolean;        // Passed Google Safe Browsing (or key not configured)
  threatType?: string;    // Present when !isSafe: e.g. "MALWARE"
